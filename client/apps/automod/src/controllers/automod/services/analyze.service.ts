...
export class AutomodAnalyzeService {
  ...
  async analyzeLastUserMessageContext(
    ...
    const username = UsersUtility.getUsername(interaction.targetMessage.author);
    const baseEmbed = new EmbedBuilder()
      .setTitle(ContextCommandAnalyzeMessage.validation.title)
      .setThumbnail(avatar)
      .setFooter({
        iconURL: avatar,
        text: username,
    ...
    await interaction.followUp({
      embeds: [
        baseEmbed
          .setTitle(ContextCommandAnalyzeMessage.awaiting.title(username))
          .setDescription(ContextCommandAnalyzeMessage.awaiting.description),
      ],
    });
    if (interaction.targetMessage.author.bot) {
      return await interaction.editReply({
        embeds: [
          baseEmbed
            .setTitle(ContextCommandAnalyzeMessage.validation.title)
            .setDescription(ContextCommandAnalyzeMessage.validation.bot),
        ],
      });
    }
    if (!interaction.targetMessage.content) {
      return await interaction.editReply({
        embeds: [
          baseEmbed
            .setTitle(ContextCommandAnalyzeMessage.validation.title)
            .setDescription(ContextCommandAnalyzeMessage.validation.empty),
        ],
      });
    }
    ...
    if (!analytics) {
      return await interaction.editReply({
        embeds: [
          baseEmbed
            .setTitle(ContextCommandAnalyzeMessage.validation.title)
            .setDescription(ContextCommandAnalyzeMessage.validation.internal),
        ],
      });
    }
    if (!analytics.success) {
      return await interaction.editReply({
        embeds: [
          baseEmbed
            .setTitle(ContextCommandAnalyzeMessage.validation.title)
            .setDescription(ContextCommandAnalyzeMessage.validation.request),
        ],
      });
    }
    const analyzeExplainer = new AutomodAnalyzeExplanaition();
    const explaination = analyzeExplainer.explain(analytics.data).toText();
